...
if (isset($_POST["save"])) {
    ...
    $sql = "SELECT id FROM route WHERE fee = '$ticketType'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $routeId = $row['id'];
    ...
    $busNumber = $_POST["bus_namber"];
    $busDriver = $_POST["bus_driver"];
    $seat = $_POST["seatnumber"];
    $buttonCounter = $_POST["buttonCounter"];

    $multipliedFare = $ticketType;

    for ($i = 0; $i < $buttonCounter; $i++) {
        $sql = "INSERT INTO `tiket`(`bus_id`, `trip_number`, `driver`, `route`, `disID`, `seatnumber`,  `Fee`) VALUES ('$busNumber', '$tripNumber', '$busDriver', '$routeId', '$discount', '$seat', '$multipliedFare')";

        if (!mysqli_query($conn, $sql)) {
            echo "Error saving the ticket: " . mysqli_error($conn);
            exit();
        }
    }

    $successMessage = "Tickets saved successfully";
}

$buttonCounter = isset($_POST["buttonCounter"]) ? $_POST["buttonCounter"] : 0;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <!-- Add any modal footer buttons here -->
...
<button type="submit" name="save" class="btn btn-primary" id="saveButton">Print</button>
